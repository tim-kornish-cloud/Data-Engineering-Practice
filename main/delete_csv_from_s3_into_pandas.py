...
# set boto3 instance variables and retrieve credentials to access AWS
service_name = 's3'
# get region name for aws s3 bucket instance
region_name = Cred.get_region(dbms = database, env = environment)
# get IAM user access key id
aws_access_key_id = Cred.get_access_key_id(dbms = database, env = environment)
# get IAM user sectret access key
aws_secret_access_key = Cred.get_secret_access_key(dbms = database, env = environment)

# create boto3 s3 client instance
s3_client = AWS_Utils.create_s3_client(service_name, region_name, aws_access_key_id, aws_secret_access_key)

# delete file from s3 buckets
response = AWS_Utils.delete_dataframe_in_s3(s3_client, s3_bucket_name, filename)
